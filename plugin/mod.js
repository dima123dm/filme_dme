(function() {
    ...
    console.log('[Rezka] Plugin loading (Smart Navigation Edition + Memory + My Folders)...');

    // --- ГЛОБАЛЬНОЕ ХРАНИЛИЩЕ ДЛЯ ЗАПОМИНАНИЯ ВЫБОРА ---
    var STORAGE_KEY = 'rezka_movie_choices';
    var FOLDERS_STORAGE_KEY = 'rezka_my_folders';
    ...
    function getFolders() {
        try {
            var data = localStorage.getItem(FOLDERS_STORAGE_KEY);
            return data ? JSON.parse(data) : {};
        } catch(e) {
            console.error('[Folders] Read error:', e);
            return {};
        }
    }
    ...
    function saveFolders(folders) {
        try {
            ...
            console.error('[Folders] Write error:', e);
        }
    }
    ...
    function createFolder(folderName) {
        ...
        if (!folders[folderName]) {
            ...
            saveFolders(folders);
            console.log('[Folders] ✅ Created:', folderName);
            return true;
        }
        return false;
    }

    function addMovieToFolder(movie, folderName) {
        var folders = getFolders();
        ...
        var exists = folder.movies.find(function(m) {
        ...
        if (!exists) {
            ...
            saveFolders(folders);
            Lampa.Noty.show('✅ Добавлено в "' + folderName + '"');
            return true;
        } else {
            Lampa.Noty.show('⚠️ Уже в папке "' + folderName + '"');
            return false;
        }
    }

    function removeMovieFromFolder(movieId, folderName) {
        var folders = getFolders();
        var folder = folders[folderName];
        
        if (folder && folder.movies) {
            folder.movies = folder.movies.filter(function(m) {
                return m.id !== movieId;
            });
            
            saveFolders(folders);
            Lampa.Noty.show('🗑️ Удалено из "' + folderName + '"');
            return true;
        }
        return false;
    }

    function isMovieInFolder(movieId) {
        var folders = getFolders();
        var inFolders = [];
        ...
        Object.keys(folders).forEach(function(folderName) {
            var folder = folders[folderName];
            if (folder.movies) {
                var found = folder.movies.find(function(m) {
                    return m.id === movieId;
                });
                if (found) {
                    inFolders.push(folderName);
                }
            }
        });
    ...
    // --- ИНТЕГРАЦИЯ С КАРТОЧКОЙ ФИЛЬМА ---
    function enhanceMovieCard() {
        Lampa.Listener.follow('full', function(data) {
            if (data.type === 'complite' && data.data && data.data.movie) {
                setTimeout(function() {
                    var movie = data.data.movie;
                    var fullContainer = $(data.object.activity.render());
                    
                    // Ищем контейнер с кнопками
                    var buttonsContainer = fullContainer.find('.full-start-new__buttons, .full-start__buttons');
                    
                    if (buttonsContainer.length) {
                        // Проверяем, добавлена ли уже наша кнопка
                        if (buttonsContainer.find('.folder-button').length === 0) {
                            // Создаем кнопку "Мои папки"
                            var folderButton = $('<div class="full-start__button selector folder-button"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg><span>Мои папки</span></div>');
                            
                            // Проверяем, в каких папках находится фильм
                            var inFolders = isMovieInFolder(movie.id);
                            if (inFolders.length > 0) {
                                folderButton.append('<span class="folder-indicator" style="margin-left: 5px;">📁 ' + inFolders.length + '</span>');
                            }
                            
                            // Обработчик клика
                            folderButton.on('hover:enter', function(e) {
                                if (e) e.preventDefault();
                                showFolderMenu(movie);
                            });
                            
                            // Добавляем кнопку в контейнер
                            buttonsContainer.append(folderButton);
                            
                            console.log('[Folders] ✅ Button added to movie card');
                        }
                    }
                }, 300);
            }
    ...
    function showFolderMenu(movie) {
        ...
        if (folderNames.length > 0) {
            folderNames.forEach(function(folderName) {
                var folder = folders[folderName];
                var isInFolder = inFolders.indexOf(folderName) !== -1;
                
                items.push({
                    title: (isInFolder ? '✅ ' : '📁 ') + folderName + ' (' + (folder.movies ? folder.movies.length : 0) + ')',
                    value: folderName,
                    selected: isInFolder,
                    folder: folder
                });
        ...
        // Опция создания новой папки
        items.push({
            title: '➕ Создать новую папку',
            value: 'create_new'
        });
        
        // Опция просмотра всех папок
        if (folderNames.length > 0) {
            items.push({
                title: '📂 Просмотреть мои папки',
                value: 'view_all'
            });
        }
            ...
            onSelect: function(item) {
                if (item.value === 'create_new') {
                    ...
                    Lampa.Input({
                        ...
                        free: true,
                        nosave: true,
                        onSelect: function(folderName) {
                            if (folderName && folderName.trim()) {
                                folderName = folderName.trim();
                                if (createFolder(folderName)) {
                                    addMovieToFolder(movie, folderName);
                                } else {
                                    Lampa.Noty.show('⚠️ Папка уже существует');
                                }
                            }
                ...
                } else if (item.value === 'view_all') {
                    // Показываем список всех папок
                    showAllFolders();
                } else if (item.value !== 'separator') {
                    // Добавляем или удаляем из папки
                    if (item.selected) {
                        removeMovieFromFolder(movie.id, item.value);
                    } else {
                        addMovieToFolder(movie, item.value);
                    }
                    Lampa.Controller.toggle('full_start');
                }
            },
    ...
    function showAllFolders() {
        ...
        if (folderNames.length === 0) {
            Lampa.Noty.show('📂 У вас пока нет папок');
            return;
        }
        
        var items = folderNames.map(function(name) {
            var folder = folders[name];
            return {
                title: '📁 ' + name + ' (' + (folder.movies ? folder.movies.length : 0) + ' фильмов)',
                value: name,
                folder: folder
            };
        });
        ...
        Lampa.Select.show({
            ...
            items: items,
            onSelect: function(item) {
                showFolderContent(item.value, item.folder);
            },
            onBack: function() {
                Lampa.Controller.toggle('full_start');
            }
    ...
    function showFolderContent(folderName, folder) {
        if (!folder || !folder.movies || folder.movies.length === 0) {
            Lampa.Noty.show('📂 Папка пуста');
            return;
        }
        
        var items = folder.movies.map(function(movie) {
            var typeIcon = movie.type === 'tv' ? '📺' : '🎬';
            return {
                title: typeIcon + ' ' + movie.title + (movie.year ? ' (' + movie.year + ')' : ''),
                description: movie.rating ? '⭐ ' + movie.rating.toFixed(1) : '',
                value: movie.id,
                movie: movie
            };
        });
        
        // Добавляем опции управления
        items.push({ title: '────────────────', value: 'separator', separator: true });
        items.push({ title: '🗑️ Очистить папку', value: 'clear_folder' });
        items.push({ title: '✏️ Переименовать папку', value: 'rename_folder' });
        items.push({ title: '❌ Удалить папку', value: 'delete_folder' });
        
        Lampa.Select.show({
            title: '📁 ' + folderName,
            items: items,
            onSelect: function(item) {
                if (item.value === 'clear_folder') {
                    // Очищаем папку
                    clearFolder(folderName);
                } else if (item.value === 'rename_folder') {
                    // Переименовываем папку
                    renameFolder(folderName);
                } else if (item.value === 'delete_folder') {
                    // Удаляем папку
                    deleteFolder(folderName);
                } else if (item.value !== 'separator' && item.movie) {
                    // Открываем карточку фильма
                    Lampa.Activity.push({
                        component: 'full',
                        id: item.movie.id,
                        method: item.movie.type,
                        source: 'tmdb',
                        card: { id: item.movie.id, source: 'tmdb' }
                    });
                }
            },
            onBack: function() {
    ...
    function clearFolder(folderName) {
        var folders = getFolders();
        if (folders[folderName]) {
            folders[folderName].movies = [];
            saveFolders(folders);
            Lampa.Noty.show('🗑️ Папка очищена');
            showAllFolders();
        }
    }
        ...
        Lampa.Input({
            ...
            onSelect: function(newName) {
                if (newName && newName.trim() && newName !== oldName) {
                    ...
                    if (folders[newName]) {
                        Lampa.Noty.show('⚠️ Папка с таким именем уже существует');
                    } else {
                        folders[newName] = folders[oldName];
                        folders[newName].name = newName;
    ...
    function init() {
        ...
        // Инициализируем функционал папок
        enhanceMovieCard();
        console.log('[Folders] ✅ Enhanced movie cards');
        ...
        setTimeout(function() {
            ...
            var icon_folders  = '<svg width="20px" height="20px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>';
            ...
            // Добавляем пункт "Мои папки" в меню
            var foldersMenuItem = $('<li class="menu__item selector" data-action="my_folders"><div class="menu__ico">' + icon_folders + '</div><div class="menu__text">Мои папки</div></li>');
            foldersMenuItem.on('hover:enter', function() { 
                showAllFolders(); 
            });
            menu.append(foldersMenuItem);
            
            console.log('[Folders] ✅ Menu item added');
        }, 1000);
